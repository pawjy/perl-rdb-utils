...
use Test::MySQL::CreateDatabase qw(
    ...
    execute_alter_tables_from_file
);
use Test::MoreMore;
...
sub _extract_schema_sql_from_file : Test(1) {
    ...
    eq_or_diff $result, [
        ...
        qq"CREATE TABLE foo (\n  bar int, baz int\n)",
        'INSERT INTO foo (bar, baz) VALUES (1, 4)',
        qq{ALTER TABLE foo ADD COLUMN abc int\nADD COLUMN xyz TINYINT},
    ];
}
...
sub _execute_alter_tables_from_file : Test(1) {
    reset_db_set;
    my $dsn = test_dsn 'mytest';
    my $dbh = dsn2dbh $dsn;
    my $f = file(__FILE__)->dir->subdir('data')->file('mysql-createdatabase-schema1.sql');
    ...
    my $f2 = file(__FILE__)->dir->subdir('data')->file('mysql-createdatabase-alters1.sql');
    execute_alter_tables_from_file $f2 => $dbh;

    my $sth = $dbh->prepare('SHOW CREATE TABLE hoge1');
    $sth->execute;
    my $data = $sth->fetchall_arrayref;
    like $data->[0]->[1], qr{added_column};
}
